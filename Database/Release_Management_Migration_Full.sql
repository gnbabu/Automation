/*============================================================================
  OHPNM Release Management - FULL Database Migration (Phase 1 + Phase 2 combined)
  Target database: MES_AUT_AI   Schema: aut

  This is the single, canonical migration script matching the CURRENT
  application implementation exactly (not an aspirational/superset schema).
  Use THIS file for new deployments; the separate Phase1/Phase2 files are
  kept only as a historical record of how the schema evolved, including
  objects that were later removed (see Release_Management_Migration_Notes.md).
  ...
  Includes (matches the implemented backend/UI exactly):
    - Release table enhancements: Version, EnvironmentId, ReleaseFolderPath,
      ModifiedBy, ActivatedBy/On, widened ReleaseLifecycle, uniqueness on
      (ReleaseName, Version, EnvironmentId).
    - New tables: ReleaseNotification, ReleaseSignOff.
    - TestCaseAssignment: nullable ReleaseId/EnvironmentId (text columns retained).
    - Non-destructive data back-fill + a mapping report for unresolved items.
    - All Release stored procedures currently used by the app: Create/Update/
      GetAll/GetById/Activate/SignOff/SignOffHistory, folder-path setter,
      delete, and notification CRUD.

  Explicitly NOT included (removed — not part of current functionality):
    - aut.ReleaseDLL table and its usp_ReleaseDLL_* procedures. DLLs are
      placed into the release folder by the existing controlled
      build/deployment process; the application never uploads, stores, or
      version-validates DLLs. Release readiness is computed entirely by the
      application layer (IReleaseReadinessService, via reflection over the
      release folder), so no DLL-tracking table/procedures exist. If an
      earlier copy of this script already created them, this script removes
      them (guarded: only if the table is empty).

  Principles:
    - NON-DESTRUCTIVE for real data: no DROP of tables/columns that could
      hold real data, no data loss. (The one exception is the guarded,
      empty-only removal of the unused ReleaseDLL table noted above.)
...
PRINT '== Full Release Management migration starting ==';
GO
...
/*============================================================================
  2. REMOVE aut.ReleaseDLL and its procedures (NOT part of current functionality)

  DLLs are placed into the release folder by the existing controlled
  build/deployment process — the application never uploads, stores, or
  version-validates DLLs, and never had a "required DLL" management UI.
  Release readiness is computed entirely by the application layer
  (IReleaseReadinessService, via reflection over the release folder), so a
  DLL-tracking table/procedures are unnecessary. This step removes the
  earlier draft of that table/procedures if a prior copy of this script
  created them. Guarded so it only drops the table if it is empty (defensive
  — this table has never been written to by the application).
============================================================================*/

IF OBJECT_ID('aut.ReleaseDLL', 'U') IS NOT NULL
BEGIN
    IF (SELECT COUNT(*) FROM aut.[ReleaseDLL]) = 0
    BEGIN
        DROP PROCEDURE IF EXISTS aut.usp_ReleaseDLL_Add;
        DROP PROCEDURE IF EXISTS aut.usp_ReleaseDLL_Update;
        DROP PROCEDURE IF EXISTS aut.usp_ReleaseDLL_GetByRelease;
        DROP PROCEDURE IF EXISTS aut.usp_ReleaseDLL_SetUpload;
        DROP PROCEDURE IF EXISTS aut.usp_ReleaseDLL_SetValidation;
        DROP PROCEDURE IF EXISTS aut.usp_ReleaseDLL_Delete;
        DROP TABLE aut.[ReleaseDLL];
        PRINT 'Removed aut.ReleaseDLL and its procedures (unused, empty)';
    END
    ELSE
    BEGIN
        PRINT 'WARNING: aut.ReleaseDLL still has rows — not dropped. Review manually.';
    END
END
GO
...
-- 7.5 usp_ActivateRelease  (guards: env active, folder present. DLL readiness is
-- NOT checked here — DLLs are filesystem state, not tracked in SQL. The
-- application (IReleaseReadinessService) verifies usable DLL content exists in
-- the release folder BEFORE calling this procedure.)
...
-- 7.8 usp_CreateOrUpdateAssignmentWithTestCases  (add optional ReleaseId/EnvironmentId)
...
PRINT '== Full Release Management migration completed ==';
GO
